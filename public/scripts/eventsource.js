    ...
    for (var i = 0; i < octets.length; i += 1) {
      ...
      if (bitsNeeded !== 0) {
        if (
          octet < 128 ||
          octet > 191 ||
          !valid((codePoint << 6) | (octet & 63), bitsNeeded - 6, octetsCount(bitsNeeded, codePoint))
        ) {
          bitsNeeded = 0;
          codePoint = REPLACER;
  ...
  XHRWrapper.prototype.send = function () {
    ...
    if (
      (!('ontimeout' in XMLHttpRequest.prototype) ||
        (!('sendAsBinary' in XMLHttpRequest.prototype) && !('mozAnon' in XMLHttpRequest.prototype))) &&
      document != undefined &&
  ...
  function start(es, url, options) {
    ...
    var xhr =
      isFetchSupported && TransportOption == undefined
        ? undefined
        : new XHRWrapper(TransportOption != undefined ? new TransportOption() : getBestXHRTransport());
    var transport =
            ...
            if (c === '\r'.charCodeAt(0) || c === '\n'.charCodeAt(0)) {
              if (state !== FIELD_START) {
                ...
                var value = chunk.slice(
                  valueStart + (valueStart < position && chunk.charCodeAt(valueStart) === ' '.charCodeAt(0) ? 1 : 0),
                  position
                );
                if (field === 'data') {
                  dataBuffer += '\n';
          ...
          onFinish(
            new Error(
              'No activity within ' +
                ...
                ' milliseconds.' +
                ' ' +
                (currentState === CONNECTING ? 'No response received.' : textLength + ' chars received.') +
                ' ' +
                'Reconnecting.'
            )
          );
